  ...
  <div class="guest-list">
    ...
    <div class="stats-row">
      ...
      <div class="stat-card">
        ...
        <div class="stat-value money">
          {{ formatMoney(store.totalGiftMoney) }}
        </div>
      </div>
      <div class="stat-card">
    ...
    <div class="toolbar">
      ...
      <div class="toolbar-btns">
        <button class="btn-danger btn-clear" @click="clearAll">
          刪除全部賓客
        </button>
        <button class="btn-primary btn-add" @click="showAddModal = true">
          ＋ 新增賓客
        </button>
      </div>
    </div>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th @click="setSort('tableNumber')">
              桌號 <SortIcon field="tableNumber" />
            </th>
            <th @click="setSort('giftMoney')">
              禮金 <SortIcon field="giftMoney" />
            </th>
            <th @click="setSort('needsCake')">
              需要禮餅 <SortIcon field="needsCake" />
            </th>
            <th @click="setSort('cakeReceived')">
              禮餅狀態 <SortIcon field="cakeReceived" />
            </th>
        ...
        <tbody>
          ...
          <tr
            v-for="g in filtered"
            :key="g.id"
            :class="{ 'row-received': g.cakeReceived }"
          >
            <td class="name-cell">
              {{ g.name }}
            </td>
            ...
            <td class="center">
              ...
              <span v-else class="tag tag-sm tag-done tag-readonly"
                >已領過</span
              >
            </td>
            <td class="center">
              <button class="btn-danger btn-sm" @click="remove(g.id)">
                刪除
              </button>
            </td>
          </tr>
...
const tables = computed(() => {
  const set = new Set(
    store.allGuests.map((g) => g.tableNumber).filter(Boolean),
  );
  return [...set].sort();
});
...
const filtered = computed(() => {
  ...
  if (filterCake.value === "received")
    list = list.filter((g) => g.needsCake && g.cakeReceived);
  else if (filterCake.value === "pending")
    list = list.filter((g) => g.needsCake && !g.cakeReceived);
...
const SortIcon = {
  ...
  setup(props) {
    return () => {
      ...
      return h(
        "span",
        { style: "font-size:0.75rem; opacity: " + (active ? 1 : 0.4) },
        icon,
      );
    };
  },
